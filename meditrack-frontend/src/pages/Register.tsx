...
const Register = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('success');
      if (role === 'PATIENT') {
        setMessage(`Compte patient créé ! Votre ID d'enregistrement est : ${response.data.registrationNumber}. Utilisez-le avec votre mot de passe "password" pour votre première connexion.`);
      } else {
        ...
        <AnimatePresence mode="wait">
          {status === 'success' ? (
            ...
            >
              ...
              <div className="form-group">
                <label>{role === 'PATIENT' ? 'Mot de passe (facultatif - défaut à "password")' : 'Mot de passe'}</label>
                <div className="input-wrapper">
                  <Lock size={18} />
                  <input
                    type="password"
              ...
              <button type="submit" className="submit-btn" disabled={status === 'loading'}>
                {status === 'loading' ? 'Traitement...' : role === 'PATIENT' ? 'S\'inscrire comme Patient' : 'S\'inscrire comme Professionnel'}
                <UserPlus size={20} />
              </button>
            </motion.form>
          )}
